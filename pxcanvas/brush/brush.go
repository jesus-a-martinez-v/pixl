...
func TryBrush(state *apptype.State, canvas apptype.Brushable, ev *desktop.MouseEvent) bool {
	switch {
	case state.BrushType == Pixel:
		return TryPaintPixel(state, canvas, ev)
	default:
		return false
	}
}

func TryPaintPixel(state *apptype.State, canvas apptype.Brushable, ev *desktop.MouseEvent) bool {
	x, y := canvas.MouseToCanvasXY(ev)
	if x != nil && y != nil && ev.Button == desktop.MouseButtonPrimary {
		canvas.SetColor(state.BrushColor, *x, *y)
		return true
	}
